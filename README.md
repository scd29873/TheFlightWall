...
> **This repository** is a fork of [FeatherKing/TheFlightWall_OSS](https://github.com/FeatherKing/TheFlightWall_OSS), set up for **four 64×64 panels in a 4×1 row (256×64)** on either of two driver boards. For the **Waveshare ESP32-S3-RGB-Matrix** (the default build), start with **[docs/waveshare-s3-matrix-4x1.md](docs/waveshare-s3-matrix-4x1.md)**: the board, power, installing the tools, flashing from a browser or PlatformIO, and debugging over USB. For the **Adafruit MatrixPortal S3**, and for everything on the panel side (chain order, frame, flicker, first-time setup, your own receiver, units, and what this fork changes), see **[docs/matrixportal-s3-4x1.md](docs/matrixportal-s3-4x1.md)**. Everything below is upstream's README and still applies.
